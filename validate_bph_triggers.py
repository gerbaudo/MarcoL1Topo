...
def main():
    ...
    for k in possible_outcomes: #initialize the histograms, they will still be empty after 
        histos[k] = {
            ...
            'n_pairs_2m9_0dr15'        : R.TH1F('n_pairs_2m9_0dr15'+'_'+k        , l+'; N 2m9_0dr15 pairs'               , *num_binning),
            ...
            'dr_2m9_0dr15'             : R.TH1F('dr_2m9_0dr15'+'_'+k             , l+'; #DeltaR 2m9_0dr15 pairs'         , *binning_2im9),
            ...
            'pt_2m9_0dr15'             : R.TH1F('pt_2m9_0dr15'+'_'+k             , l+'; #Pt 2m9_0dr15 muons'             , *pt_binning),            
    ...
    for iEvent, event in enumerate(chain):
        ...
        list_mu6mu4 = algo_MU6MU4(muons) #mu6mu4 list
        list_2m9_0dr15_pairs, list_pairs, list_2m9_0dr15= algo_2M9_0DR15(muons, muonList=True) #2im9_0dr15 couplelist
        list_mu6mu4_2m9_0dr15_pairs, list_mu6mu4_pairs = algo_2M9_0DR15([muon for muon, Pt in list_mu6mu4]) #mu6_2m9_0dr15 couplelist
        list_2m9_0dr15_mu6mu4 = algo_MU6MU4_pairs(list_2m9_0dr15_pairs)
        ...
        fill_histos(histos[outcome], histos2[outcome], muons, list_mu6mu4, list_2m9_0dr15,
                    list_2m9_0dr15_pairs, list_pairs, list_mu6mu4_2m9_0dr15_pairs, list_mu6mu4_pairs) #fill histograms
...
def algo_2M9_0DR15(muons, muonList=False): #retuns ordered list with any couple of muons satisfying 0DR15
    couples_any   = []
    n_mu = len(muons)
    for i in range(n_mu-1): #check all muon couples to see if Delta r is lower than 1.5
        for j in range(i+1,n_mu):
            ...
            m = (muons[i].p4+muons[j].p4).M()
            couples_any.append((muons[i],muons[j], dr, m))
    
    couples_any.sort(key = lambda couple: couple[2]) #sort list
    couples_2m9_0dr15 = [couple for couple in couples_any if couple[2]<1.505 and (couple[3]>1950 and couple[3]<9050)] #take only 2mu9_0dr15
    
    if muonList: #return a list of the muons that belong to at least one couple of couples_2m9_0dr15
        list_2m9_0dr15 = []
        for couple in couples_any:
            if couple[0] in list_2m9_0dr15:
                pass
            else:
                list_2m9_0dr15.append(couple[0])
            
            if couple[1] in list_2m9_0dr15:
                pass
            else:
                list_2m9_0dr15.append(couple[1])
...
def algo_MU6MU4(muons): #returns list with all muons satifying MU6 sorted by energy
    mu6ab_list = []
...
def fill_histos(histos, histos2, muons, list_mu6mu4, list_2m9_0dr15, list_2m9_0dr15_pairs,
                list_pairs, list_mu6mu4_2m9_0dr15_pairs, list_mu6mu4_pairs): #fills histograms
    n_mu = len(muons)
    n_mu6mu4= len(list_mu6mu4)
    n_2m9_0dr15_pairs = len(list_2m9_0dr15_pairs)
    n_mu6mu4_pairs = len(list_mu6mu4_pairs)
    n_mu6mu4_2m9_0dr15_pairs = len(list_mu6mu4_2m9_0dr15_pairs)
    n_pairs = len(list_pairs)
    n_2m9_0dr15 = len (list_2m9_0dr15)
    histos['n_mu'                    ].Fill(n_mu)
    histos['n_mu6mu4'                ].Fill(n_mu6mu4)
    ...
    histos['n_pairs_2m9_0dr15'       ].Fill(n_2m9_0dr15_pairs)         #number of 2m9_0dr15 pairs
    ...
    if n_2m9_0dr15_pairs: #fill histograms of dr
        for couple in list_2m9_0dr15_pairs:
            histos['dr_2m9_0dr15'].Fill(couple[2])
    ...
    for muon in list_2m9_0dr15: #fill histogram of momentums
        histos['pt_2m9_0dr15'].Fill(muon.p4.Pt())

  
    for muon, pt in list_mu6mu4: #fill histograms of angles
        Phi = muon.p4.Phi()
        Eta = muon.p4.Eta()
